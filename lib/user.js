...
User.prototype.anonymousId = function(anonymousId) {
  ...
  // set / remove
  if (arguments.length) {
    store.set('hrv_ajs_anonymous_id', anonymousId);
    this._setAnonymousIdInLocalStorage(anonymousId);
    return this;
  }

  // new
  anonymousId = store.get('hrv_ajs_anonymous_id');
  if (anonymousId) {
  ...
  if (!this._options.localStorageFallbackDisabled) {
    ...
    anonymousId = localStorage.get('hrv_ajs_anonymous_id');
    if (anonymousId) {
  ...
  store.set('hrv_ajs_anonymous_id', anonymousId);
  this._setAnonymousIdInLocalStorage(anonymousId);
  return store.get('hrv_ajs_anonymous_id');
};
...
User.prototype._setAnonymousIdInLocalStorage = function(id) {
  if (!this._options.localStorageFallbackDisabled) {
    localStorage.set('hrv_ajs_anonymous_id', id);
  }
};
